...
class DDPG(object):
	...
	def train(self, replay_buffer, iterations, batch_size=32, discount=0.99, tau=0.001):
		...
		for it in range(iterations):
			...
			state = torch.FloatTensor(x).cuda()
			action = torch.FloatTensor(u).cuda()
			next_state = torch.FloatTensor(y).cuda()
			done = torch.FloatTensor(1 - d).cuda()
			reward = torch.FloatTensor(r).cuda()
